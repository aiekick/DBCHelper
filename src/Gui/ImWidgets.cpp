...
bool ImGui::RadioButtonLabeled(float vWidth, const char* label, bool active, bool disabled, float align_x)
{
	...
	if (label_size.x > 0.0f)
	{
		...
		RenderTextClipped(total_bb.Min, total_bb.Max, label, nullptr, &label_size, ImVec2(align_x, 0.5f));
		if (pushed)
			ImGui::PopStyleColor();
	}
...
bool ImGui::RadioButtonLabeled(float vWidth, const char* label, const char* help, bool active, bool disabled, ImFont* vLabelFont, float vAlignX)
{
	if (vLabelFont)	ImGui::PushFont(vLabelFont);
	const bool change = RadioButtonLabeled(vWidth, label, active, disabled, vAlignX);
...
bool ImGui::ContrastedSmallButton(const char* label)
{
	const bool pushed = ImGui::PushStyleColorWithContrast(ImGuiCol_Button, ImGuiCol_Text, CustomStyle::puContrastedTextColor, CustomStyle::puContrastRatio);

	ImGui::PushID(++CustomStyle::pushId);

	const bool res = ImGui::SmallButton(label);
