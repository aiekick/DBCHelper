...
namespace ImGui
{
	...
	IMGUI_API bool RadioButtonLabeled(float vWidth, const char* label, bool active, bool disabled, float align_x);
	IMGUI_API bool RadioButtonLabeled(float vWidth, const char* label, const char* help, bool active, bool disabled = false, ImFont* vLabelFont = nullptr, float vAlignX = 0.5f);
	IMGUI_API bool RadioButtonLabeled(float vWidth, const char* label, const char* help, bool* active, bool disabled = false, ImFont* vLabelFont = nullptr,	float vAlignX = 0.5f);
	template<typename T>
	IMGUI_API bool RadioButtonLabeled_BitWize(
		...
		float vAlignX = 0.5f) // radio witl use only theses flags
	{
		bool selected = (int32_t)*vContainer & (int32_t)vFlag;
		const bool res = RadioButtonLabeled(vWidth, vLabel, vHelp, &selected, vDisableSelection, vLabelFont, vAlignX);
		if (res) {
			if (selected) {
				if (vOneOrZeroAtTime) {
					if ((int32_t)vFlagsToTakeIntoAccount) {
						if ((int32_t)vFlag & (int32_t)vFlagsToTakeIntoAccount) {
							*vContainer = (T)((int32_t)*vContainer & ~(int32_t)vFlagsToTakeIntoAccount); // remove these flags
							*vContainer = (T)((int32_t)*vContainer | (int32_t)vFlag); // add
						}
					}
					...
					if ((int32_t)vFlagsToTakeIntoAccount) {
						if ((int32_t)vFlag & (int32_t)vFlagsToTakeIntoAccount) {
							(int32_t)*vContainer = (T)((int32_t)*vContainer & ~(int32_t)vFlagsToTakeIntoAccount); // remove these flags
							(int32_t)*vContainer = (T)((int32_t)*vContainer | (int32_t)vFlag); // add
						}
					}
					else *vContainer = (T)((int32_t)*vContainer | (int32_t)vFlag); // add
				}
			}
			else {
				if (vOneOrZeroAtTime) {
				...
				else *vContainer = (T)((int32_t)*vContainer & ~(int32_t)vFlag); // remove one
			}
		}
		return res;
	}
	...
	IMGUI_API bool RadioButtonLabeled_BitWize(
		...
		float vAlignX = 0.5f) // radio witl use only theses flags
	{
		bool selected = (int32_t)*vContainer & vFlag;
		const char* label = (selected ? vLabelOK : vLabelNOK);
		const bool res = RadioButtonLabeled(vWidth, label, vHelp, &selected, vDisableSelection, vLabelFont, vAlignX);
		if (res) {
			if (selected) {
				if (vOneOrZeroAtTime) {
					if ((int32_t)vFlagsToTakeIntoAccount) {
	...
	IMGUI_API bool MenuItem(const char* label, const char* shortcut, T* vContainer, T vFlag, bool vOnlyOneSameTime = false)
	{
		...
		if (res) {
			if (selected) {
				if (vOnlyOneSameTime) {
					(int32_t)*vContainer = (int32_t)vFlag; // set
				}
				else {
					(int32_t)*vContainer = (T)((int32_t)*vContainer | (int32_t)vFlag);// add
				}
				...
				if (!vOnlyOneSameTime) {
					(int32_t)*vContainer = (T)((int32_t)*vContainer & ~(int32_t)vFlag); // remove
				}
			}
		}
		return res;
	...
	IMGUI_API bool ContrastedSmallButton(const char* label);
